var CanvasUtil= {
    ...
    getImageOptions: function (options) {
        options = (options) ? options : {};
        var defaultOptions = {
            url: '',
            sx: 0,
    ...
    drawImages: function (id, totalImgs) {
        var self = this;
        var ctx = self.getContext(id);
        var totalImgsLegth = totalImgs.length;
        if (totalImgsLegth === 0) return;
        for (var i = 0; i < totalImgsLegth; i++) {
            var tmpImg = totalImgs[i];
            var imageOptions = self.getImageOptions(tmpImg);
            var url = imageOptions.url,
                sx = imageOptions.sx,
                sy = imageOptions.sy,
                swidth = imageOptions.swidth,
                sheight = imageOptions.sheight,
                x = imageOptions.x,
                y = imageOptions.y,
                width = imageOptions.width,
                height = imageOptions.height;
            var tmpImgUrl = tmpImg.url;
            var image = new Image();
            image.src = tmpImgUrl;
            image.onload = (function(image_p, sx_p, sy_p, swidth_p, sheight_p, x_p, y_p, width_p, height_p) {
                return function() {ctx.drawImage(image_p, sx_p, sy_p, swidth_p, sheight_p, x_p, y_p, width_p, height_p);}
